...
import Bbc.Addr (addrToHiLo,addrOfHiLo)
import Bbc.Byte (Byte)
import Bbc.Six502.Operations (Op(..),Instruction,Mode(..),Arg(..))
import qualified Bbc.Six502.OpCode as OpCode(table)
...
spec2 :: ModeSpec
spec2 = (2, \case
            lo:hi:rest -> (ArgAddr $ addrOfHiLo hi lo, rest)
            [lo] -> (ArgAddr $ addrOfHiLo 0 lo, [])
            _ -> (ArgAddr 0,[]))
